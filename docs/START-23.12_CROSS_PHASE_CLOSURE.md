...
- The Phase 3B canonical future producers `workshop` and `scheduler` are recognized by the Phase 4 caller registry.
...
1. **Future producer caller mismatch** — Phase 3B froze `workshop` and `scheduler`, while Phase 4 did not recognize those caller IDs.
