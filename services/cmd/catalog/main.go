...
const partnerModuleSelect = `SELECT
	...
	pm.entitlement_state,pm.commercial_configured,pm.contract_currency,pm.quote_reference,pm.commercial_effective_at,pm.entitlement_source,pm.plan_key,
...
func scanPartnerModule(s scanner, locale string) (map[string]any, error) {
	var id, k, labelEN, labelHU, g, groupEN, groupHU, st, entitlementState, contractCurrency, quoteReference, entitlementSource, planKey, publicationStatus, implementationState, currency, v, lv, availability, priceSource, activationSource string
	...
	err := s.Scan(
		...
		&entitlementState,&commercialConfigured,&contractCurrency,&quoteReference,&commercialEffectiveAt,&entitlementSource,&planKey,
