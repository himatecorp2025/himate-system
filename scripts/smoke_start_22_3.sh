...
register_billing_evidence_document "$COOKIE" "$partner_id" "INVOICE" "INVOICE" "Activation Fee Invoice" "start223-invoice" >/dev/null
...
register_billing_evidence_document "$COOKIE" "$partner_id" "PAYMENT_EVIDENCE" "OTHER" "Activation payment receipt" "start223-payment" >/dev/null
...
printf '%s' "$commercial_state" | python3 -c 'import json,sys; d=json.load(sys.stdin); assert d["partner_id"]==sys.argv[1]; assert d["tenant_scope"]=="CREDENTIAL_BOUND"; assert d["contract_version"]=="START-22.3"; assert d["design_contract_version"]=="START-23.8"; assert d["design"]["profile_id"]=="theme_classic_editorial"; assert d["design"]["theme"]["layout_key"]=="classic_editorial"; assert d["domain_binding"]["primary_domain"]=="start223.example.com"; assert "ci.commercial_snapshot" in d["entitlements"]; assert set(d["entitlements"]["ci.commercial_snapshot"].keys())=={"status","version","included_in_base"}; assert "price" not in d and "billing" not in d and "invoices" not in d and "payments" not in d; assert "updated_by" not in d["adapter"] and "updated_at" not in d["adapter"]; dc=d["data_contract"]; assert dc["privacy_mode"]=="AGGREGATED_ONLY"; assert dc["raw_invoice_data_allowed"] is False and dc["raw_payment_data_allowed"] is False; assert dc["retention_policy"]=="7_YEARS"' "$partner_id"
echo ok
