...
printf '%s' "$events" | python3 -c 'import json,sys; d=json.load(sys.stdin); types={x["event_type"] for x in d["items"]}; required={"MODULE_CANCELLATION_SCHEDULED","MODULE_CANCELLATION_WITHDRAWN","MODULE_PERIOD_ENDED"}; assert required <= types,(required-types,d)'
echo ok
