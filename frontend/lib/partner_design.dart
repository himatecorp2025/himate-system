...
extension PartnerDesignUI on _PartnerPortalShellState {
  ...
  Color get workspaceTextColor =>
      partnerThemeColor(workspaceSettings['text_color'], brandText);
  ...
  IconData workspaceIconData(String key) {
    switch (key) {
      ...
      case 'crm': return Icons.people_alt_outlined;
      ...
      case 'documents': return Icons.folder_copy_outlined;
      case 'settings': return Icons.settings_outlined;
      case 'integrations': return Icons.hub_outlined;
  ...
  Widget workspaceLogo({double height = 42}) {
    if (workspaceLogoUrl.isEmpty) {
      return HimateLogo(onDark: workspaceSidebarColor.computeLuminance() < .45, width: 174);
    }
  ...
  Widget modulePresentationIcon(Map<String, dynamic> module, {double size = 23}) {
    final presentation = modulePresentationFor((module['key'] ?? '').toString());
    ...
    if (customUrl.isNotEmpty) {
      return ClipRRect(
        ...
        child: Image.network(
          ...
          height: size,
          fit: BoxFit.cover,
          errorBuilder: (_, __, ___) => Icon(Icons.extension_outlined, size: size),
        ),
      );
    }
    return Icon(
      ...
      child: Column(
        ...
        children: [
          ...
          _InfoCard(
            ...
            children: [
              _DefinitionRow(label: 'Default module', value: workspaceDefaultModuleKey.isEmpty ? 'Overview' : workspaceDefaultModuleKey),
              _DefinitionRow(label: 'Primary color', value: (workspaceSettings['primary_color'] ?? '#0B1F3B').toString()),
              _DefinitionRow(label: 'Sidebar color', value: (workspaceSettings['sidebar_color'] ?? '#06172C').toString()),
              _DefinitionRow(label: 'Background color', value: (workspaceSettings['background_color'] ?? '#F8F9FB').toString()),
              _DefinitionRow(label: 'Accent color', value: (workspaceSettings['accent_color'] ?? '#D4AF6B').toString()),
              _DefinitionRow(label: 'Customized modules', value: modulePresentations.length.toString(), emphasis: modulePresentations.isNotEmpty),
              const _DefinitionRow(label: 'System contract', value: 'Presentation only · canonical HIMATE mechanics unchanged'),
            ],
          ),
...
String _partnerShortHash(String value) {
  ...
  Future<void> editWorkspacePersonalization() async {
    ...
    final name = TextEditingController(
      text: (workspace['workspace_name'] ?? company['display_name'] ?? '').toString(),
    );
    final primary = TextEditingController(text: (workspace['primary_color'] ?? '#0B1F3B').toString());
    final sidebar = TextEditingController(text: (workspace['sidebar_color'] ?? '#06172C').toString());
    final background = TextEditingController(text: (workspace['background_color'] ?? '#F8F9FB').toString());
    final accent = TextEditingController(text: (workspace['accent_color'] ?? '#D4AF6B').toString());
    final textColor = TextEditingController(text: (workspace['text_color'] ?? '#1F2937').toString());
    var logoMediaID = (workspace['logo_media_id'] ?? '').toString();
    var defaultModuleKey = (workspace['default_module_key'] ?? '').toString();
    String? validationError;

    final activeModules = modules.where((module) =>
      (module['access_state'] ?? '').toString() == 'ACTIVE' && module['executable'] == true
    ).toList();
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setLocal) {
          final knownLogo = logoMediaID.isEmpty ||
              designMedia.any((item) => (item['id'] ?? '').toString() == logoMediaID);
          final knownDefault = defaultModuleKey.isEmpty ||
              activeModules.any((item) => (item['key'] ?? '').toString() == defaultModuleKey);
          return BrandDialog(
            title: 'Workspace identity',
            subtitle: 'Customize presentation only. HIMATE module keys, APIs, routes, billing, permissions, entitlements and workflows remain unchanged.',
            icon: Icons.dashboard_customize_outlined,
            width: 900,
            primaryLabel: 'Save workspace',
            onPrimary: () {
              final colors = [primary.text, sidebar.text, background.text, accent.text, textColor.text];
              String? problem;
              ...
              } else if (_workspaceContrastRatio(background.text, textColor.text) < 4.5) {
                problem = 'Background and text colors need at least 4.5:1 contrast.';
              }
              if (problem != null) {
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: uiLiteral('Changes the tenant workspace label, not the HIMATE system name.'),
                  ),
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  value: knownLogo ? logoMediaID : '',
                  decoration: InputDecoration(labelText: uiLiteral('Workspace logo')),
                  items: <DropdownMenuItem<String>>[
                    const DropdownMenuItem(value: '', child: LText('HIMATE / company default')),
                    for (final item in designMedia)
                      DropdownMenuItem(
                        value: (item['id'] ?? '').toString(),
                        child: LText((item['original_filename'] ?? item['id'] ?? '').toString()),
                      ),
                  ],
                  onChanged: (value) => setLocal(() => logoMediaID = value ?? ''),
                ),
                const SizedBox(height: 12),
                ResponsiveFieldPair(
                  first: TextField(controller: primary, decoration: InputDecoration(labelText: uiLiteral('Primary color'))),
                  second: TextField(controller: accent, decoration: InputDecoration(labelText: uiLiteral('Accent color'))),
                ),
                const SizedBox(height: 12),
                ResponsiveFieldPair(
                  first: TextField(controller: sidebar, decoration: InputDecoration(labelText: uiLiteral('Sidebar color'))),
                  second: TextField(controller: background, decoration: InputDecoration(labelText: uiLiteral('Background color'))),
                ),
                const SizedBox(height: 12),
                TextField(
                  controller: textColor,
                  decoration: InputDecoration(
                    ...
                    helperText: uiLiteral('HIMATE enforces readable text/background contrast.'),
                  ),
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<String>(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: uiLiteral('Only an ACTIVE module owned by your organization can be selected.'),
                  ),
                  items: <DropdownMenuItem<String>>[
                    const DropdownMenuItem(value: '', child: LText('Overview')),
                  ...
                  onChanged: (value) => setLocal(() => defaultModuleKey = value ?? ''),
                ),
              ],
            ),
          );
    ...
    for (final controller in [name, primary, sidebar, background, accent, textColor]) {
      controller.dispose();
    }
  }
  ...
  Future<void> editModulePresentation(Map<String, dynamic> module) async {
    ...
    final displayName = TextEditingController(text: (existing?['display_name'] ?? '').toString());
    final description = TextEditingController(text: (existing?['description'] ?? '').toString());
    final cardColor = TextEditingController(text: (existing?['card_color'] ?? '').toString());
    var iconKey = (existing?['icon_key'] ?? '').toString();
    var customIconMediaID = (existing?['custom_icon_media_id'] ?? '').toString();
    String? validationError;

    final rawLibrary = designState['icon_library'];
    final iconLibrary = rawLibrary is List
        ? rawLibrary.whereType<Map>().map((item) => Map<String, dynamic>.from(item)).toList()
        : <Map<String, dynamic>>[];
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setLocal) {
          final knownIcon = iconKey.isEmpty ||
              iconLibrary.any((item) => (item['key'] ?? '').toString() == iconKey);
          final knownMedia = customIconMediaID.isEmpty ||
              designMedia.any((item) => (item['id'] ?? '').toString() == customIconMediaID);
          return BrandDialog(
            title: 'Module presentation',
            subtitle: 'Rename and restyle the card for your team. The canonical HIMATE module identity and behavior cannot be changed.',
            ...
            onPrimary: () {
              ...
              } else if (cardColor.text.trim().isNotEmpty && !_workspaceHexValid(cardColor.text)) {
                problem = 'Card color must use the #RRGGBB format.';
              } else if (iconKey.isNotEmpty && customIconMediaID.isNotEmpty) {
                problem = 'Choose an HIMATE icon or a custom icon, not both.';
              }
              if (problem != null) {
                setLocal(() => validationError = problem);
                return;
            ...
            child: Column(
              ...
              children: [
                ...
                _DefinitionRow(label: 'Official HIMATE name', value: (module['label'] ?? moduleKey).toString(), emphasis: true),
                _DefinitionRow(label: 'Canonical module key', value: moduleKey),
                const SizedBox(height: 14),
                TextField(
                  controller: displayName,
                  decoration: InputDecoration(
                    labelText: uiLiteral('Your display name'),
                    helperText: uiLiteral('Leave blank to use the official HIMATE name.'),
                  ),
                ),
                const SizedBox(height: 12),
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: uiLiteral('Use a short 3–4 sentence description for your team.'),
                  ),
                ),
                const SizedBox(height: 12),
                ResponsiveFieldPair(
                  first: DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(labelText: uiLiteral('HIMATE icon library')),
                    items: <DropdownMenuItem<String>>[
                      const DropdownMenuItem(value: '', child: LText('HIMATE default')),
                      for (final item in iconLibrary)
                        DropdownMenuItem(
                          value: (item['key'] ?? '').toString(),
                          child: LText((item['label'] ?? item['key'] ?? '').toString()),
                        ),
                    ],
                  ...
                  second: DropdownButtonFormField<String>(
                    ...
                    decoration: InputDecoration(labelText: uiLiteral('Custom icon asset')),
                    items: <DropdownMenuItem<String>>[
                      const DropdownMenuItem(value: '', child: LText('No custom icon')),
                      for (final item in designMedia)
                        DropdownMenuItem(
                          value: (item['id'] ?? '').toString(),
                          child: LText((item['original_filename'] ?? item['id'] ?? '').toString()),
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: uiLiteral('Optional #RRGGBB color; leave blank for HIMATE default.'),
                  ),
                ),
                if (existing != null) ...[
                  const SizedBox(height: 18),
                    ...
                    onPressed: () async {
                      final confirm = await showDialog<bool>(
                        ...
                        builder: (confirmContext) => AlertDialog(
                          ...
                          content: const LText('Your custom module name, description, icon and card color will be removed. The module itself is unchanged.'),
                          actions: [
                            TextButton(onPressed: () => Navigator.pop(confirmContext, false), child: const LText('Cancel')),
                            FilledButton(onPressed: () => Navigator.pop(confirmContext, true), child: const LText('Reset')),
                          ],
                        ),
                      );
                      if (confirm != true) return;
                      try {
                        await widget.api.delete('/partner/api/v1/design/modules/$moduleKey');
                        if (dialogContext.mounted) Navigator.pop(dialogContext, false);
                        await load();
                        if (mounted) toast('Module presentation reset to HIMATE default.');
                      } catch (e) {
                        if (mounted) toast(e.toString(), failure: true);
                      }
                    },
