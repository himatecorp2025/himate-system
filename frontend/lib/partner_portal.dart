...
class _PartnerPortalShellState extends State<PartnerPortalShell> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      setState(() {
        ...
        if (loadedFrequency == 'MONTHLY' || loadedFrequency == 'ANNUAL') planBillingFrequency = loadedFrequency;
        loading = false;
      });
    } catch (e) {
  ...
  Widget billingPage() {
    return Content(
      ...
      subtitle: 'Choose monthly or annual billing, manage your plan and review provider-backed invoice history.',
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (plans.isNotEmpty) ...[
          Row(children: [
            const Expanded(child: _SectionHeader(title: 'Subscription Plans', subtitle: 'Annual pricing shows the full list price and discounted annual charge.')),
        ...
        ResponsiveKpiGrid(children: [
          ...
          Kpi(label: 'Current charge', value: money(billing['current_total']), note: hasManagedPlan ? 'Plan price' : 'Legacy total', icon: Icons.payments_outlined, accent: brandGold),
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setLocal) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const LText('Choose up to 15 modules. Later Flex module changes take effect on the next calendar-month boundary.', style: TextStyle(color: brandTextSoft, fontSize: 10.5)),
                const SizedBox(height: 12),
                ...
                const SizedBox(height: 8),
                LText('${selected.length} / 15 selected', style: const TextStyle(color: brandSuccess, fontWeight: FontWeight.w700)),
              ]),
            ),
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(dialogContext), child: const LText('Cancel')),
            FilledButton(onPressed: () => Navigator.pop(dialogContext, selected.toList()..sort()), child: const LText('Confirm modules')),
          ],
        ),
      ),
    );
  ...
  Future<void> selectSubscriptionPlan(Map<String, dynamic> target) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        content: LText('Selected billing: ${planMoney(amount)} / $periodLabel. Upgrades take effect immediately and charge the full plan-price difference. Downgrades keep the current package until its billing boundary; no refund is issued.'),
        actions: [
