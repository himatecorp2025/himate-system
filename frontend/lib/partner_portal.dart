...
class _PartnerPortalShellState extends State<PartnerPortalShell> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      setState(() {
        ...
        final frequency = '${plan['billing_frequency'] ?? ''}'.toUpperCase();
        if (frequency == 'MONTHLY' || frequency == 'ANNUAL') planBillingFrequency = frequency;
        loading = false;
      });
    } catch (e) {
  ...
  String planMoney(dynamic value) => intl.NumberFormat.currency(symbol: '\
    ...
    return Content(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ResponsiveKpiGrid(children: [
          ...
          Kpi(label: 'Current plan', value: hasManagedPlan ? '${plan['display_name'] ?? plan['plan_key']}' : money(billing['current_total']), note: hasManagedPlan ? '${plan['billing_frequency'] ?? 'MONTHLY'} billing' : 'Legacy commercial model', icon: Icons.payments_outlined, accent: brandGold),
        ...
        LayoutBuilder(builder: (context, constraints) {
          ...
          return Wrap(spacing: 12, runSpacing: 12, children: [
            ...
            SizedBox(width: width, child: _InfoCard(title: 'Next billing cycle', icon: Icons.calendar_month_outlined, children: [
              ...
              if (hasManagedPlan) _DefinitionRow(label: 'Subscription plan', value: '${plan['display_name'] ?? plan['plan_key']}'),
              if (hasManagedPlan) _DefinitionRow(label: 'Billing frequency', value: '${plan['billing_frequency'] ?? '—'}'),
              if (!hasManagedPlan) _DefinitionRow(label: 'Base service fee', value: money(billing['effective_base_fee'])),
              if (!hasManagedPlan) _DefinitionRow(label: 'Module fee', value: money(billing['extra_module_fee'])),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _DefinitionRow(
            ...
            value: hasManagedPlan
                ...
                : module['included_in_base'] == true ? 'Included in base' : '${module['currency'] ?? 'USD'} ${number(module['partner_price']).toStringAsFixed(2)}',
          ),
  ...
  Widget modulesPage() {
    ...
    return Content(
      ...
      subtitle: hasManagedPlan
          ? 'Your module access is controlled by your subscription plan. Starter and Business are fixed packages; Flex module-set changes are managed from Billing.'
  ...
  Widget billingPage() {
    return Content(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (plans.isNotEmpty) ...[
          Row(children: [
            const Expanded(child: _SectionHeader(title: 'Subscription Plans', subtitle: 'Annual prepayment displays the full list price and your discounted annual price.')),
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            return Wrap(spacing: 12, runSpacing: 12, children: [
              for (final item in plans)
                SizedBox(width: width, child: Card(child: Padding(
                  padding: const EdgeInsets.all(18),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Row(children: [
                      Expanded(child: LText('${item['display_name']}', style: const TextStyle(color: brandNavy, fontSize: 17, fontWeight: FontWeight.w800))),
                      if (currentPlanKey == '${item['plan_key']}') const _StatusPill(label: 'CURRENT'),
                    ]),
                    const SizedBox(height: 10),
                    if (planBillingFrequency == 'MONTHLY')
                      LText('${planMoney(item['monthly_price'])} / month', style: const TextStyle(color: brandGold, fontSize: 18, fontWeight: FontWeight.w800))
                    else ...[
                      if (number(item['annual_list_price']) > number(item['annual_price']))
                        Text(planMoney(item['annual_list_price']), style: const TextStyle(color: brandTextSoft, decoration: TextDecoration.lineThrough, fontWeight: FontWeight.w700)),
                      LText('${planMoney(item['annual_price'])} / year', style: const TextStyle(color: brandGold, fontSize: 18, fontWeight: FontWeight.w800)),
                      if (number(item['annual_savings']) > 0)
                        LText('Save ${planMoney(item['annual_savings'])}', style: const TextStyle(color: brandSuccess, fontSize: 10.5, fontWeight: FontWeight.w700)),
                    ],
                    const SizedBox(height: 12),
                    _DefinitionRow(label: 'Modules', value: '${item['module_limit']}'),
                    _DefinitionRow(label: 'Choice', value: item['selection_mode'] == 'SELECTABLE' ? 'Choose your modules' : 'Fixed HIMATE package'),
                    const SizedBox(height: 14),
                    SizedBox(width: double.infinity, child: currentPlanKey == '${item['plan_key']}'
                        ? OutlinedButton(onPressed: currentPlanKey == 'FLEX' && can('modules.write') ? manageFlexModules : null, child: LText(currentPlanKey == 'FLEX' ? 'Manage Flex modules' : 'Current plan'))
                        : FilledButton(onPressed: can('modules.write') && item['ready'] == true ? () => selectSubscriptionPlan(item) : null, child: const LText('Choose plan'))),
                  ]),
                ))),
            ]);
          }),
          if (hasManagedPlan && plan['scheduled_change'] != null) ...[
            const SizedBox(height: 12),
            _MessageCard(icon: Icons.schedule_outlined, title: 'Plan change scheduled', message: '${plan['scheduled_change']}'),
          ],
          const SizedBox(height: 24),
        ],
        ResponsiveKpiGrid(children: [
          Kpi(label: 'Current plan', value: hasManagedPlan ? '${plan['display_name'] ?? plan['plan_key']}' : 'Legacy', note: '${plan['billing_frequency'] ?? '—'}', icon: Icons.workspace_premium_outlined, accent: brandNavy),
          Kpi(label: 'Current charge', value: money(billing['current_total']), note: hasManagedPlan ? 'Plan price' : 'Legacy commercial total', icon: Icons.payments_outlined, accent: brandGold),
          Kpi(label: 'Module access', value: hasManagedPlan ? '${plan['module_limit'] ?? 0}' : '${modules.where((m) => m['status'] == 'ACTIVE').length}', note: hasManagedPlan ? '${plan['selection_mode'] ?? ''}' : 'Active modules', icon: Icons.extension_outlined, accent: brandSteel),
          Kpi(label: 'Next billing date', value: '${billing['next_billing_date'] ?? '—'}', note: hasManagedPlan ? '${plan['billing_frequency'] ?? ''}' : 'Legacy cycle', icon: Icons.calendar_month_outlined, accent: brandSuccess),
        ]),
        const SizedBox(height: 24),
...
, decimalDigits: 0).format(number(value));
  ...
  Future<List<String>?> chooseFlexModules({List<String>? initial}) async {
    ...
    final candidates = modules.where((m) => m['publication_status'] == 'PUBLISHED' && m['implementation_state'] == 'READY').toList();
    return showDialog<List<String>>(
      ...
      builder: (context) => StatefulBuilder(builder: (context, setLocal) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const LText('Choose up to 15 modules. A later Flex module-set change takes effect on the next calendar month boundary.', style: TextStyle(color: brandTextSoft, fontSize: 10.5)),
              const SizedBox(height: 12),
              for (final module in candidates)
                CheckboxListTile(
                  ...
                  subtitle: LText('${module['group_label'] ?? ''}', style: const TextStyle(color: brandTextSoft, fontSize: 9)),
              ...
              const SizedBox(height: 8),
              LText('${selected.length} / 15 selected', style: TextStyle(color: selected.length <= 15 ? brandSuccess : brandDanger, fontWeight: FontWeight.w700)),
            ]),
          ),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const LText('Cancel')),
          FilledButton(onPressed: () => Navigator.pop(context, selected.toList()..sort()), child: const LText('Confirm modules')),
        ],
      )),
    );
  }
  ...
  Future<void> selectSubscriptionPlan(Map<String, dynamic> target) async {
    ...
    if (targetKey == 'FLEX') {
      final current = plan['active_module_keys'] is List ? (plan['active_module_keys'] as List).map((e) => e.toString()).toList() : <String>[];
      final chosen = await chooseFlexModules(initial: current);
      if (chosen == null) return;
      moduleKeys = chosen;
    }
    final amount = planBillingFrequency == 'ANNUAL' ? number(target['annual_price']) : number(target['monthly_price']);
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: LText(
          'Selected billing: ${planMoney(amount)} / $periodLabel. Upgrades take effect immediately and charge the full plan-price difference. Downgrades keep your current package until its billing boundary; no refund is issued.',
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const LText('Cancel')),
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const LText('Confirm plan')),
        ],
      ),
    );
    if (ok != true) return;
  ...
  Future<void> manageFlexModules() async {
    ...
    if (chosen == null) return;
    try {
      final response = await widget.api.put('/partner/api/v1/plan/modules', {'module_keys': chosen, 'reason': 'Partner Portal Flex selection'});
      await load();
