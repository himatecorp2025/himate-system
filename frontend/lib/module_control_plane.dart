...
, decimalDigits: 0).format(number(value));
  ...
  Future<void> configureFixedPlan(Map<String, dynamic> plan) async {
    ...
    final candidates = modules.where((m) =>
      s(m['publication_status']) == 'PUBLISHED' && s(m['implementation_state']) == 'READY'
    ).toList();
    final ok = await showDialog<bool>(
      context: context,
      builder: (context) => StatefulBuilder(builder: (context, setLocal) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              LText('Select exactly $limit modules. Changes to an existing package take effect from the next calendar month.', style: const TextStyle(color: brandTextSoft, fontSize: 10.5)),
              const SizedBox(height: 14),
        ...
        actions: [
          ...
          FilledButton(onPressed: selected.length == limit ? () => Navigator.pop(context, true) : null, child: const LText('Save package')),
        ],
      )),
    );
    if (ok != true) return;
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (annual < annualList) ...[
            Text(planMoney(annualList), style: const TextStyle(color: brandTextSoft, decoration: TextDecoration.lineThrough, fontWeight: FontWeight.w600)),
            const SizedBox(height: 2),
          ],
          LText('${planMoney(annual)} / year', style: const TextStyle(color: brandGold, fontWeight: FontWeight.w800, fontSize: 16)),
          if (savings > 0)
            LText('Save ${planMoney(savings)} with annual prepayment', style: const TextStyle(color: brandSuccess, fontSize: 10, fontWeight: FontWeight.w700)),
          const SizedBox(height: 14),
          _DefinitionRow(label: 'Module capacity', value: '${plan['module_limit'] ?? 0}'),
          _DefinitionRow(label: 'Selection model', value: fixed ? 'Fixed by HIMATE' : s(plan['selection_mode']) == 'SELECTABLE' ? 'Partner chooses modules' : 'Custom contract'),
          if (fixed) _DefinitionRow(label: 'Configured modules', value: '${keys.length} / ${plan['module_limit'] ?? 0}'),
          if (fixed && keys.isNotEmpty) ...[
            const SizedBox(height: 8),
            Wrap(spacing: 6, runSpacing: 6, children: [
              for (final key in keys) Chip(label: LText(moduleLabel(key))),
            ]),
          ],
          ...
          else if (s(plan['plan_key']) == 'FLEX')
            const _MessageCard(
              icon: Icons.auto_awesome_outlined,
              title: 'Customer-selected package',
              message: 'Flex customers can choose up to 15 published modules. HIMATE does not define a fixed Flex module set.',
            ),
        ]),
      ),
    );
  ...
  Widget subscriptionPlansPage() {
    ...
    return Content(
      ...
      subtitle: 'Plan pricing is the recurring billing authority. Starter and Business have fixed HIMATE-defined modules; Flex lets the customer choose up to 15.',
      actions: [
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const _MessageCard(
          ...
          message: 'Starter: $500/month · Business: $1,500/month · Flex: $2,500/month. Module-level partner prices remain stored for future add-on pricing but do not drive pilot recurring invoices.',
        ),
        const SizedBox(height: 18),
        LayoutBuilder(builder: (context, constraints) {
          ...
          return Wrap(spacing: 12, runSpacing: 12, children: [
            for (final plan in visible) SizedBox(width: width, child: subscriptionPlanCard(plan)),
          ]);
        }),
